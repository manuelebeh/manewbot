...
registerCommand({
  ...
  while (list3.length > 1 && !false3) {
    ...
    for (const tmp15 of list4) {
      const value15 = wordsForRound(currentPlayer2, list);
      if (!value15.length) {
        await sock.sendMessage(chatJid, {
      ...
      const value16 = pickWord(value15, value2);
      const value17 = shuffleWord(value16);
      await sock.sendMessage(chatJid, {
      ...
      try {
        ...
        if (normalizeWord(value18) === normalizeWord(value16)) {
          tmp15.score++;
          false4 = true;
          currentPlayer3++;
          await sock.sendMessage(chatJid, {
